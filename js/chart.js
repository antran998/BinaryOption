...
jQuery.ajax({
    ...
    success: function(data){
      ...
      var countCandle=0;
      var alltime;
      var intervalTime = 0; // 1000 load of time
      ...
      depositebut.addEventListener("click",function(){
        if(parseFloat(inputmoney.value) > 0){
          ...
          var inputMoneyAmount = parseFloat(inputmoney.value);
          var depositeAmount = currMoney + inputMoneyAmount;

          var sendObj = {
            depositeAmount: inputMoneyAmount
          }

          $.ajax({        
              url: "../DemoStock/test/stockaction.php",
              method: "post",
      ...
      myBuy.addEventListener("click",function(){//////////////////////////////Click Buy button
        if(amt.value>0){// condition: the amount div has to be filled
          ...
          if(newMoney>=0){
            ...
            ////////// POST data
            var sendObj = {
              purTime: hisPurTime[countCandle-1] ,
              purPrice: markClosePrice[countCandle-1] ,
              purAmount: markAmtValue[countCandle-1],
              lever: leverChoose[countCandle-1],
              callput: "call"
            }
        ...
        if(markExist==1){
          ...
          for(var i = 0; i<=countCandle;i++){
            if(markClosePrice[i]>=0){                          
              ...
              hisPercent = hisPercent.toFixed(2);

              var sendObj = {
                closeTime: hisExpiTime,
                closePrice: currPrice,
                profit: hisProfit,
                percent: hisPercent,
                takeorlost: TakeOrLost,
                hisPurTime: hisPurTime[i]
              }
          ...
          for(var i = countCandle-1; i>=0;i--){                      
            ...
            openPrice = dataObj[i]["open"]; // load real time
            highestPrice = dataObj[i]["high"];
            lowestPrice = dataObj[i]["low"]; 
            closePrice = dataObj[i]["close"];
      ...
      var firsttime = setTimeout(flow,intervalTime);
      var animation = setInterval(flow, intervalTime+1000);// Load off time
      ...
      function flow(){
          if(countCandle>0){
            ...
            //   contentType: "application/json",
            //   success: function(data){
            //     time = data['Meta Data']['3. Last Refreshed']; //////////// run on realtime
            ...
            ////////////////////////// get data but not send request for off time
            openPrice = parseFloat(data['Time Series (1min)'][time]['1. open']); // Load off time
            highestPrice = parseFloat(data['Time Series (1min)'][time]['2. high']);
            lowestPrice = parseFloat(data['Time Series (1min)'][time]['3. low']);
            closePrice = parseFloat(data['Time Series (1min)'][time]['4. close']);
          ...
          openPrice = dataObj[countCandle]["open"]; // load real time
          highestPrice = dataObj[countCandle]["high"];
          lowestPrice = dataObj[countCandle]["low"]; 
          closePrice = dataObj[countCandle]["close"];     
...
function historyTable(hisPurTime,hisExpiTime,openingPrice,closingPrice,hisAmt,hisProfit,hisPercent,TakeOrLost){
  ...
  histo.insertBefore(newdiv, histo.childNodes[0]);
...
function logout() {
  document.cookie = "empty=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;";
  window.open('../Demostock/index.php','_self');
}
